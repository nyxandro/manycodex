...
- `/oai` или `/oai list` — показать список сохрадённых аккаунтов.
- `/oai save <name>` — сохранить текущий активный аккаунт.
  - Если имя уже занято, команда вернет ошибку (защита от перезаписи).
- `/oai load <name>` — переключить активный аккаунт.
  - Также работают алиасы: `/oai use <name>` или просто `/oai <name>`.
- `/oai del <name>` — удалить сохраненный аккаунт.
  - Также работают алиасы: `/oai rm <name>` или `/oai d <name>`.
